...
CHANGELOG = [
    ("0.16.0", "2026-06-13", "16:00 JST", [
        "「⊡ 全体表示にフィット」機能を追加: ロボット・STL・砥石・刃先・経路・"
        "参照フレーム・マーカーの全ジオメトリが画面に収まるよう視点とズームを"
        "自動調整（RoboDK の Fit 相当）",
        "3Dビューポート上部のボタン・表示メニュー・F キーのいずれからも実行可能"
        "（テキスト入力中の F は通常の文字入力を優先）",
    ]),
